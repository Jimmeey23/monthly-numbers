...
function testDashboardCockpitHeaderContracts() {
  ...
  assert(html.includes('backdrop-filter:blur(18px) saturate(132%)'), 'network cards should use a restrained glassmorphic background');
  ...
  assert(html.includes('background:linear-gradient(145deg,rgba(4,10,28,.74),rgba(16,24,48,.42))!important'), 'cockpit cards should use a darker executive metric surface');
  assert(html.includes('border-left:3px solid var(--cockpit-tone)!important'), 'cockpit cards should use a restrained tone rail instead of large color fills');
  assert(html.includes('grid-template-columns:minmax(0,1fr) minmax(86px,31%)!important'), 'cockpit cards should use a compact metric/sparkline layout');
  ...
  assert(html.includes('font-variant-numeric:tabular-nums!important'), 'cockpit card values should use tabular numerals');
  assert(html.includes('border-radius:999px!important'), 'cockpit card secondary context should be styled as compact pills');
  assert(html.includes('linear-gradient(90deg,rgba(255,255,255,.075) 1px,transparent 1px)'), 'cockpit sparklines should sit in a subtle chart lane');
